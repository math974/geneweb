"""Use case pour export vers répertoire."""
...
class ExportToDirectoryUseCase:
    """
    Use case: Exporter vers un répertoire.
    
    Exporte les données vers un répertoire avec une structure organisée
    (fichier principal, sous-répertoires pour personnes/familles, etc.).
    """
    ...
    def execute(self, request: ExportRequest) -> ExportResult:
        """
        Exécute l'export vers répertoire.
        ...
        try:
            ...
            if request.options.output_dir:
                ...
                output_dir = Path("export_directory")
            
            # 5. Exporter vers répertoire
            ...
            return ExportResult(
                success=False,
                exported_persons=0,
                error_message=f"Erreur lors de l'export vers répertoire: {str(e)}"
            )
