#!/usr/bin/env python3
"""
Script de test Golden Master étendu pour le binaire Python GWU.

Ce script teste le binaire Python contre TOUS les golden masters OCaml
pour vérifier la compatibilité complète avec toutes les combinaisons d'options.
"""
...
class ExtendedGoldenMasterTester:
    """Testeur de Golden Master étendu pour GWU Python."""
    ...
    def test_all_scenarios(self) -> None:
        """Teste tous les scénarios de golden master étendus."""
        print(f"🧪 TEST GOLDEN MASTER ÉTENDU POUR LA BASE: {self.base_name}")
        print("=" * 70)
        ...
        scenarios = [
            ...
            # Tests de combinaisons simples (2 options)
            {"name": "Clé + Ascendance 1", "args": ["-k", "Jean Pierre.0 Galichet", "-a", "1"]},
            {"name": "Clé + Descendance 1", "args": ["-k", "Jean Pierre.0 Galichet", "-d", "1"]},
            {"name": "Clé + Ascendance+Descendance 1", "args": ["-k", "Jean Pierre.0 Galichet", "-ad", "1"]},
            {"name": "Clé + Sans notes", "args": ["-k", "Jean Pierre.0 Galichet", "--no-notes"]},
            {"name": "Clé + Sans notes ni sources", "args": ["-k", "Jean Pierre.0 Galichet", "--no-notes", "--no-src"]},
            {"name": "Ascendance 1 + Sans notes", "args": ["-a", "1", "--no-notes"]},
            {"name": "Descendance 1 + Sans notes", "args": ["-d", "1", "--no-notes"]},
            {"name": "Ascendance+Descendance 1 + Sans notes", "args": ["-ad", "1", "--no-notes"]},
            {"name": "Personnes isolées + Sans notes", "args": ["--isolated", "--no-notes"]},
            {"name": "Personnes isolées + Sans notes ni sources", "args": ["--isolated", "--no-notes", "--no-src"]},
            {"name": "Séparation + Sans notes", "args": ["--sep", "Jean Pierre.0 Galichet", "--no-notes"]},
            {"name": "Séparation + Format old-gw", "args": ["--sep", "Jean Pierre.0 Galichet", "--old-gw"]},
            {"name": "Encodage ASCII + Format old-gw", "args": ["-enc", "ASCII", "--old-gw"]},
            {"name": "Encodage ASCII + Sans notes", "args": ["-enc", "ASCII", "--no-notes"]},
            {"name": "Format old-gw + Ascendance 1", "args": ["--old-gw", "-a", "1"]},
            {"name": "Format old-gw + Descendance 1", "args": ["--old-gw", "-d", "1"]},
            
            # Tests de combinaisons complexes (3+ options)
            {"name": "Clé + Ascendance 1 + Descendance 1 + Sans notes", "args": ["-k", "Jean Pierre.0 Galichet", "-a", "1", "-d", "1", "--no-notes"]},
            {"name": "Clé + Ascendance+Descendance 2 + Sans notes ni sources", "args": ["-k", "Jean Pierre.0 Galichet", "-ad", "2", "--no-notes", "--no-src"]},
            {"name": "Personnes isolées + Sans notes + Sans sources", "args": ["--isolated", "--no-notes", "--no-src"]},
            {"name": "Encodage ASCII + Format old-gw + Sans notes", "args": ["-enc", "ASCII", "--old-gw", "--no-notes"]},
        ]
    ...
    def _print_summary(self) -> None:
        ...
        print("\n" + "=" * 70)
        print("📊 RÉSUMÉ DES TESTS GOLDEN MASTER ÉTENDUS")
        print("=" * 70)
...
def main():
    ...
    tester = ExtendedGoldenMasterTester(base_name)
